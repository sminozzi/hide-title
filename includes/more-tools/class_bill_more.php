...
{
    // require_once WPTOOLSPATH . "includes/checkup/class_bill_catch_errors.php";
...
function cardealer_upinstall() {

    /*
        $plugin_dir_url = plugin_dir_url(__FILE__);
        //require_once(WPMEMORYPATH . "includes/more/more.php");
        $style_handle = md5_file(plugin_dir_path(__FILE__);
        wp_enqueue_style($style_handle, $plugin_dir_url . 'includes/more/more.css');
        wp_register_script($style_handle.'js', $plugin_dir_url . 'includes/more/more.js', array('jquery'));
        wp_enqueue_script($style_handle.'js');
        require_once(plugin_dir_path(__FILE__) . "includes/more/more.php");
    */

        $style_handle = substr(md5_file(plugin_dir_path(__FILE__) . '/more.css'), 0, 8);
        wp_enqueue_style($style_handle, plugin_dir_url(__FILE__) . '/more.css');
        wp_register_script($style_handle . '-js', plugin_dir_url(__FILE__) . '/more/more.js', array('jquery'));
        wp_enqueue_script($style_handle . '-js');
        // require_once(plugin_dir_path(__FILE__) . "includes/more/more.php");


}
add_action("wp_init", "cardealer_upinstall");
...
class Bill_show_more_plugins
{
    ...
    */
    ...
    public function bill_plugin_installed($slug) {
        // Use a função global get_plugins() com a barra invertida
    ...
    ?>
        ...
        <?php wp_nonce_field( 'bill_install_plugin_class', 'nonce_install' ); 
...
$plugin_displayer = new Bill_show_more_plugins();
//$plugin_displayer->show_plugins();
//$plugin_install->bill_install_plugin();
if( !function_exists('bill_install_plugin2')) {
    function bill_install_plugin2()
    {
        if (isset($_POST["nonce"])) {
            $nonce = sanitize_text_field($_POST["nonce"]);
            if (!wp_verify_nonce($nonce, "bill_install_plugin_class") and !wp_verify_nonce($nonce, "bill_install")    ) {
        ...
        if (is_wp_error($api)) {
            ...
            class bill_install_QuietSkin extends \WP_Upgrader_Skin
            {
            ...
            $skin = new bill_install_QuietSkin(["api" => $api]);
